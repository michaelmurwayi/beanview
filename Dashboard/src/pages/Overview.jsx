...
const Overview = () => {
  ...
  useEffect(() => {
    ...
    let filteredRecords = coffeeRecords;

    if (filters.grade) {
      filteredRecords = filteredRecords.filter(r => r.grade === filters.grade);
    }

    if (filters.mark) {
      filteredRecords = filteredRecords.filter(r => r.mark === filters.mark);
    }
    ...
    if (filters.outturn) {
      filteredRecords = filteredRecords.filter(r => r.outturn === filters.outturn);
    }

    // 👇 Logic to show 1 farmer if mark is selected
    ...
    const totalSales = new Set(filteredRecords.map(r => r.sale)).size;
    const uniqueGrades = new Set(filteredRecords.map(r => r.grade)).size;
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} sm={6} md={4}>
            <Card
              ...
              text="All sales attended to date"
            />
          </Grid>
          <Grid item xs={12} sm={6} md={4}>
            <Card
        ...
        >
          ...
          >
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', }}>
              <Button variant="contained" sx={{color: '#121330'}} onClick={handleResetFilters}>
                Reset
              </Button>
            </Box>
          </Box>
